...
def LeConstantesEContaConstantes():
    ...
    vetor_constantes = [
        caracter for caracter in vetor if caracter in constantes]
...
def LeNotasDezAlunos(notas_alunos=[], num_alunos=1):
    ...
    if num_alunos == 10:
        ...
        print(
            f'# Numero de alunos com media maior ou igual a 7.0: {len(media_maior_igual_7)}')
...
def LeDezNumeros():
    ...
    print(f'# Vetor: {vetor}')
    print(
        f'# Soma dos quadrados dos elementos do vetor: {sum(list_quadrados)}')
...
def LeDoisVetoresDezElementos():
    ...
    vetor_c = []
    [[vetor_c.append(vetor_a[i]), vetor_c.append(vetor_b[i])]
